...
- Each tap has a small **base chance** (`Balance.baseEnergyTapChance`, `0.1%`) to bank a burst of
  Energy. **Fishing's *Big Catch* perk multiplies that chance** (×1 at Lv 1 → ×10 at Lv 99, so up
...
5. **Raids** — one raid per skill category, whose tier scales with the group's **average** level.
   ...
   **final boss**) sharing one countdown and one pool of **raid HP**. Every one of the **twelve
   ...
   rhythm, heat charge-and-release, vent-pressure bleeding, stealth looting, path-tracing, rune memory,
