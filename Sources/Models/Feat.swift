...
/// A just-completed reward, surfaced by the UI as a celebratory toast. Either a single Feat, a
/// batch of Feats completed together, or a Diary-tier completion — carrying the Tokens awarded.
struct FeatEvent: Identifiable, Equatable {
    let id = UUID()
    let title: String
    let subtitle: String
    let tokens: Int
...
/// Difficulty tier of a Feat within its Diary. Higher tiers pay out more Reward Tokens.
/// The Token values themselves are centralized in `Balance.Rewards` so re-balancing never
/// touches this file.
enum FeatTier: String, Codable, CaseIterable, Identifiable {
    ...
    /// Reward Tokens granted for completing a single Feat of this tier.
    var tokenReward: Int {
        switch self {
...
/// A themed set of Feats. Mirrors OSRS Achievement Diaries, re-themed around the account's
/// actual activities (there are no regions in XP Waste).
enum FeatDiary: String, Codable, CaseIterable, Identifiable {
...
/// The kind of event that can advance a Feat's progress. Feats are indexed by trigger so a hook
/// only re-checks the handful of Feats that could actually have changed — no per-frame scans.
enum FeatTrigger: Hashable {
...
/// they persist compactly in `featCounters` and can't drift from the Feat catalog.
enum FeatCounter {
...
/// (a one-shot Feat uses `goal == 1`); the Feat is complete once progress reaches the goal.
///
...
struct Feat: Identifiable {
    ...
    let detail: String
    let goal: Int
    /// Events that can change this Feat's progress.
    let triggers: Set<FeatTrigger>
    /// Current progress toward `goal` (unclamped; the engine/UI clamp for display).
    let progress: Progress

    /// True when this Feat tracks a running count worth showing as "n / goal" (goal > 1).
    var isCounter: Bool { goal > 1 }

    var tokenReward: Int { tier.tokenReward }
}

/// The static Feat catalog and lookup indexes. Pure data — like `TrainingMethod` flavor, it holds
/// no state. Progress is computed live from `GameState`.
enum FeatCatalog {

    /// Every Feat in the game, grouped by Diary in declaration order.
    static let all: [Feat] = combat + production + utility + gathering + idler + tycoon + completionist

    /// Feats for one Diary, in tier order.
    static func feats(in diary: FeatDiary) -> [Feat] {
        all.filter { $0.diary == diary }.sorted { $0.tier.order < $1.tier.order }
    }

    /// Feats registered for a given trigger (for cheap, targeted evaluation).
    static let byTrigger: [FeatTrigger: [Feat]] = {
        var map: [FeatTrigger: [Feat]] = [:]
        for feat in all {
            for trigger in feat.triggers { map[trigger, default: []].append(feat) }
        }
        return map
    }()

    /// Feats sharing a (diary, tier) group — used to award the diary-tier completion bonus.
    static func group(_ diary: FeatDiary, _ tier: FeatTier) -> [Feat] {
    ...
    static let production: [Feat] = [
        ...
        Feat(id: "prod.all99", diary: .production, tier: .master,
    ...
    static let utility: [Feat] = [
        Feat(id: "util.combo120", diary: .utility, tier: .easy,
             title: "Nimble", detail: "Build a ×1.2 tap combo.",
             goal: 120, triggers: [.combo]) { $0.featCounter(FeatCounter.bestComboBips) },
        Feat(id: "util.combo140", diary: .utility, tier: .medium,
             title: "In the Zone", detail: "Build a ×1.4 tap combo.",
             goal: 140, triggers: [.combo]) { $0.featCounter(FeatCounter.bestComboBips) },
        Feat(id: "util.refund", diary: .utility, tier: .medium,
             title: "Light Fingers", detail: "Pickpocket back a coupon or Supercharge.",
             goal: 1, triggers: [.refund]) { $0.featCounter(FeatCounter.refunds) },
        Feat(id: "util.level60", diary: .utility, tier: .hard,
             title: "Untouchable", detail: "Reach level 60 in any utility skill.",
             goal: 60, triggers: [.levelUp]) { maxLevel(in: .utility, $0) },
        Feat(id: "util.tier6", diary: .utility, tier: .elite,
             title: "Shadow", detail: "Unlock a tier-6 method on any utility skill.",
             goal: 1, triggers: [.levelUp]) { hasTier6(in: .utility, $0) },
        Feat(id: "util.all99", diary: .utility, tier: .master,
             title: "Utility Legend", detail: "Reach level 99 in all five utility skills.",
             goal: 5, triggers: [.levelUp]) { maxedCount(in: .utility, $0) }
    ]

    static let gathering: [Feat] = [
        Feat(id: "gather.energyFull", diary: .gathering, tier: .easy,
    ...
    static let idler: [Feat] = [
        Feat(id: "idle.slot1", diary: .idler, tier: .easy,
             title: "First Slot", detail: "Assign a skill to an AFK slot.",
             goal: 1, triggers: [.slot]) { $0.slots.count },
        Feat(id: "idle.return1", diary: .idler, tier: .easy,
             title: "Well Rested", detail: "Return from an offline session.",
             goal: 1, triggers: [.offlineReturn]) { $0.featCounter(FeatCounter.offlineReturns) },
        Feat(id: "idle.slot2", diary: .idler, tier: .medium,
             title: "Double Duty", detail: "Fill two AFK slots at once.",
             goal: 2, triggers: [.slot]) { $0.slots.count },
        Feat(id: "idle.offline100k", diary: .idler, tier: .medium,
             title: "Overnight", detail: "Earn 100k XP in a single offline return.",
             goal: 100_000, triggers: [.offlineReturn]) { $0.featCounter(FeatCounter.bestOfflineXP) },
        Feat(id: "idle.slot5", diary: .idler, tier: .hard,
             title: "Fully Staffed", detail: "Fill all five AFK slots at once.",
             goal: 5, triggers: [.slot]) { $0.slots.count },
        Feat(id: "idle.offline500k", diary: .idler, tier: .elite,
             title: "Big Sleeper", detail: "Earn 500k XP in a single offline return.",
             goal: 500_000, triggers: [.offlineReturn]) { $0.featCounter(FeatCounter.bestOfflineXP) },
        Feat(id: "idle.total1000", diary: .idler, tier: .master,
             title: "Idle Empire", detail: "Reach total level 1000.",
             goal: 1000, triggers: [.levelUp]) { $0.totalLevel }
    ]

    static let tycoon: [Feat] = [
        Feat(id: "tycoon.boost1", diary: .tycoon, tier: .easy,
             title: "First Boost", detail: "Activate an XP Boost.",
             goal: 1, triggers: [.boost]) { $0.featCounter(FeatCounter.boosts) },
        Feat(id: "tycoon.cell1", diary: .tycoon, tier: .easy,
             title: "Recharged", detail: "Use an Energy Cell.",
             goal: 1, triggers: [.energyCell]) { $0.featCounter(FeatCounter.energyCells) },
        Feat(id: "tycoon.stack", diary: .tycoon, tier: .medium,
             title: "Double Trouble", detail: "Tap with a Boost and Supercharge both active.",
             goal: 1, triggers: [.tap]) { $0.featCounter(FeatCounter.stackedBursts) },
        Feat(id: "tycoon.coupons5", diary: .tycoon, tier: .medium,
             title: "Coupon Hoarder", detail: "Hold five Boost Coupons at once.",
             goal: 5, triggers: [.currency]) { $0.doubleXPCoupons },
        Feat(id: "tycoon.boost10", diary: .tycoon, tier: .hard,
             title: "Power User", detail: "Activate 10 XP Boosts.",
             goal: 10, triggers: [.boost]) { $0.featCounter(FeatCounter.boosts) },
        Feat(id: "tycoon.cells25", diary: .tycoon, tier: .elite,
             title: "Energy Baron", detail: "Use 25 Energy Cells.",
             goal: 25, triggers: [.energyCell]) { $0.featCounter(FeatCounter.energyCells) },
        Feat(id: "tycoon.boost50", diary: .tycoon, tier: .master,
             title: "Tycoon", detail: "Activate 50 XP Boosts.",
             goal: 50, triggers: [.boost]) { $0.featCounter(FeatCounter.boosts) }
    ]

    static let completionist: [Feat] = [
        Feat(id: "comp.total250", diary: .completionist, tier: .medium,
             title: "Journeyman", detail: "Reach total level 250.",
             goal: 250, triggers: [.levelUp]) { $0.totalLevel },
        Feat(id: "comp.first99", diary: .completionist, tier: .hard,
             title: "First 99", detail: "Reach level 99 in any skill.",
             goal: 1, triggers: [.levelUp]) { $0.maxedSkillCount },
        Feat(id: "comp.total750", diary: .completionist, tier: .hard,
             title: "Veteran", detail: "Reach total level 750.",
             goal: 750, triggers: [.levelUp]) { $0.totalLevel },
        Feat(id: "comp.combat", diary: .completionist, tier: .elite,
             title: "Combat Cape", detail: "Reach level 99 in every combat skill.",
             goal: 6, triggers: [.levelUp]) { maxedCount(in: .combat, $0) },
        Feat(id: "comp.production", diary: .completionist, tier: .elite,
             title: "Production Cape", detail: "Reach level 99 in every production skill.",
             goal: 7, triggers: [.levelUp]) { maxedCount(in: .production, $0) },
        Feat(id: "comp.utility", diary: .completionist, tier: .elite,
             title: "Utility Cape", detail: "Reach level 99 in every utility skill.",
             goal: 5, triggers: [.levelUp]) { maxedCount(in: .utility, $0) },
        Feat(id: "comp.gathering", diary: .completionist, tier: .elite,
             title: "Gathering Cape", detail: "Reach level 99 in every gathering skill.",
             goal: 5, triggers: [.levelUp]) { maxedCount(in: .gathering, $0) },
        Feat(id: "comp.maxcape", diary: .completionist, tier: .master,
             title: "Max Cape", detail: "Reach level 99 in all 23 skills.",
             goal: SkillID.allCases.count, triggers: [.levelUp]) { $0.maxedSkillCount },
        Feat(id: "comp.eternal", diary: .completionist, tier: .master,
             title: "Eternal", detail: "Reach the 200M XP ceiling in any skill.",
             goal: 1, triggers: [.levelUp]) { $0.maxXPSkillCount },
        Feat(id: "comp.truecomp", diary: .completionist, tier: .master,
